```{r, setup, echo=F}
#placeholder for simulations data file used by brew
fig_width = 7.00

opts_chunk$set(results='hide',highlight=F, cache=T, tidy=F, echo=F, message=F, warning=F, comment=NA, external=F,split=F, fig.width=fig_width, fig.height=fig_width/2, dpi=320, dev='png', dev.args = list(type='cairo'))
...
### Descriptives
```{r, results='asis'}
tabular(congruency + 1 ~ (Heading('Accuracy')*accuracy+(Heading('RT')*accuracyf + 1) *(Heading()*RTs+Heading('RM ESR')*ecr+Heading('CM ESR')*cm_esr))*Heading()*mymean, data = dt_by_trial)
```
...
for (i in 1:length(sim_data)){
  ...
  p1 <- qplot(data=data.frame(energies_dt[!is.na(respN),list(energy=mymean(energy)), by=list(resp_lockedN,congruency,accuracyf)]), x=resp_lockedN,y=energy, linetype=accuracyf, color=congruency, geom='line', size=I(1))+theme_minimal() + ggtitle('R-locked')+scale_x_continuous(breaks=seq(-50,50,by=10))
  ...
  p3 <- qplot(data=data.frame(energies_dt[!is.na(respN),list(energy=mymean(energy)), by=list(resp_lockedN,accuracyf)]), x=resp_lockedN,y=energy, linetype=accuracyf,  geom='line', size=I(1))+theme_minimal()+scale_x_continuous(breaks=seq(-50,50,by=10)) + ggtitle('R-locked')
